    ...
    unsigned char *more)
{
    ...
    if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, tmp)) {
        // Serial_Printf("i2c_read FAILED at %d:", __LINE__);
        // Serial_Printf("i2c_read(st.hw->addr=0x%x, st.reg->fifo_count_h=%d, 2, tmp)\n", st.hw->addr, st.reg->fifo_count_h);
        return -1;
    }
    fifo_count = (tmp[0] << 8) | tmp[1];
    if (fifo_count < length) {
        more[0] = 0;
        // Serial_Printf("FIFO count: %hd < %d\n", fifo_count, length);
        return -1;
    }
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        if (i2c_read(st.hw->addr, st.reg->int_status, 1, tmp)) {
            // Serial_Printf("i2c_read failed at %d\n", __LINE__);
            return -1;
        }
        if (tmp[0] & BIT_FIFO_OVERFLOW) {
            mpu_reset_fifo();
            // Serial_Printf("FIFO overflow!\n");
            return -2;
        }
    }
...
int mpu_run_self_test(long *gyro, long *accel)
{
    ...
    // Serial_Printf("accel_res = %d, gyro_res = %d\n", accel_result, gyro_result);
...
#else
        // result |= 0x04;
        // Serial_Printf("WTF are you doing here\n");
#endif
restore:
#elif defined MPU6500
...
uint8_t mpu_dmp_get_quat(double *pitch,double *roll,double *yaw, long q[4]) {
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		*pitch = (double)asin(-2.0 * q1 * q3 + 2.0 * q0* q2)* 57.3;	// pitch
		*roll  = (double)atan2(2.0 * q2 * q3 + 2.0 * q0 * q1, -2.0 * q1 * q1 - 2.0 * q2* q2 + 1.0)* 57.3;	// roll
		*yaw   = (double)atan2(2.0*(q1*q2 + q0*q3), q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
